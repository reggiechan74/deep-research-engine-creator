...
const SHUTDOWN_DELAY_MS = 5 * 60 * 1000;
...
function broadcastStatus() {
  ...
  if (!shutdownScheduled && status.pipeline && status.pipeline.phases) {
...
try {
  ...
  console.error('Warning: could not watch status directory:', err.message);
}
